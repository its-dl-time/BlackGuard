...
export interface BehaviorEvaluationInput {
  riskScore: number; // 0 → 1, from ML
  orderType: "buy" | "sell";
  horizon: "short" | "medium" | "long";
  recentActions: number; // số lệnh gần đây
...
export class RuleService {
  evaluateBehaviorViolation(input: BehaviorEvaluationInput): BehaviorEvaluationOutput {
    ...
    if (violationProb < 0.4) severity = "low";
    else if (violationProb < 0.7) severity = "medium";
    else severity = "high";

    // Cooldown logic
    let cooldownMinutes: number;
    if (riskScore < 0.4) cooldownMinutes = 0;
    else if (riskScore < 0.6) cooldownMinutes = 15;
    else if (riskScore < 0.8) cooldownMinutes = 30;
  ...
  private getExplanation(violationType: ViolationType): string[] {
